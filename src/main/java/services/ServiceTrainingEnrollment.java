...
public class ServiceTrainingEnrollment implements IService<TrainingEnrollment> {
    ...
    public void supprimer(int id) throws SQLException {
        ...
        PreparedStatement ps = connection.prepareStatement("DELETE FROM training_enrollments WHERE id=?");
        ps.setInt(1, id);
        ps.executeUpdate(); ps.close();
    }
    ...
    public List<TrainingEnrollment> getByUser(int userId) throws SQLException {
        ...
        List<TrainingEnrollment> list = new ArrayList<>();
        PreparedStatement ps = connection.prepareStatement("SELECT * FROM training_enrollments WHERE user_id=? ORDER BY enrolled_at DESC");
        ps.setInt(1, userId);
    ...
    public List<TrainingEnrollment> getByCourse(int courseId) throws SQLException {
        ...
        List<TrainingEnrollment> list = new ArrayList<>();
        PreparedStatement ps = connection.prepareStatement("SELECT * FROM training_enrollments WHERE course_id=? ORDER BY enrolled_at DESC");
    ...
    public void updateProgress(int enrollId, int progress) throws SQLException {
        ...
        String sql;
        if (progress >= 100) {
            sql = "UPDATE training_enrollments SET progress=100, status='COMPLETED', completed_at=NOW() WHERE id=?";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setInt(1, enrollId);
            ps.executeUpdate(); ps.close();
        } else {
            sql = "UPDATE training_enrollments SET progress=?, status='IN_PROGRESS' WHERE id=?";
            PreparedStatement ps = connection.prepareStatement(sql);
